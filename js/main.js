...
import {
    ...
    niveauListe,
    genreListe,
    SET_SCORE_FIRST_TEAM_KEY,
    SET_SCORE_SECOND_TEAM_KEY
} from './values.js'
    ...
    switch (currentPage) {
        ...
        case pages.EXECUTION_TOURNOI:
            ...
            for (var i = 0; i < storage.tournoi.tours.length; i++) {
                ...
                body.appendChild(buildTour(storage.tournoi.tours[i], i));
            }
            break;
    }
    ...
    switch (currentPage) {
        ...
        case pages.MODIFICATION_PREPARATION:
            ...
            for (var t in typeTournoiListe) {
                elementsTypeTournoi.push({ "id": t, "name": "typeTournoi", "value": typeTournoiListe[t], "checked": storage.tournoi.typeTournoi === typeTournoiListe[t] });
            }
            ...
            for (var t in modeTournoiListe) {
                ...
                elementsModeTournoi.push({ "id": t, "name": "modeTournoi", "value": modeTournoiListe[t], "checked": storage.tournoi.modeTournoi === modeTournoiListe[t] });
            }
...
function buildHeaderTour(i) {
    ...
    } else if (storage.tournoi.currentTour > i) {
...
function buildTour(tour, i) {
    ...
    if (storage.tournoi.currentTour == i) globalTour.classList.add("currentTour");
    else if (storage.tournoi.currentTour > i) globalTour.classList.add("closedTour");
...
function buildMatch(match, j) {
    ...
    matchDom.appendChild(listFirstTeam);
...
function buildHeaderJoueur() {
    ...
    switch (currentPage) {
        case pages.ACCUEIL:
            ...
            var ss2 = MH.makeSpan(storage.getNbJoueurSelected() + "/" + storage.joueurs.length, "nbSsTitle");
            ssTitle.appendChild(ss1);
            ssTitle.appendChild(ss2);
            header.appendChild(ssTitle);
            header.appendChild(MH.makeButton({
...
function buildHeaderContrainte() {
    ...
    switch (currentPage) {
        case pages.MODIFICATION_PREPARATION:
            ...
            var ss2 = MH.makeSpan(storage.getNbContrainteActif() + "/" + storage.tournoi.contraintes.length, "nbSsTitle");
            ssTitle.appendChild(ss1);
            ssTitle.appendChild(ss2);
            header.appendChild(ssTitle);
            header.appendChild(MH.makeButton({
...
function buildListContraintes() {
    ...
    var flag = false;
    var compt = 1;
    for (var i = 0; i < storage.tournoi.contraintes.length; i++) {
        switch (currentPage) {
            case pages.MODIFICATION_PREPARATION:
                if (storage.tournoi.contraintes[i].actif && !storage.tournoi.contraintes[i].disabled) {
                    listContraintes.appendChild(buildContrainte(storage.tournoi.contraintes[i], i, compt));
                    flag = true;
                    compt++;
                }
                break;
...
function buildContrainte(contrainte, i, compt) {
    ...
    switch (currentPage) {
        case pages.MODIFICATION_PREPARATION:
            ...
            if (contrainte.name == "LIMITPOINT") div.appendChild(MH.makeSpan(storage.tournoi.limitPoint));
        ...
        case pages.MODIFICATION_CONTRAINTES:
            ...
            if (contrainte.name == "LIMITPOINT") {
                contrainteDom.appendChild(MH.makeInput("number", { "id": "limitPoint", "value": storage.tournoi.limitPoint }));
            }
            ...
            if (i == storage.tournoi.contraintes.length - 1) descendre.classList.add("disabled");
...
function buildListJoueur() {
    ...
    if (currentPage == pages.MODIFICATION_JOUEUR) {
        ...
        if (storage.joueurs.length == 0) {
            ...
            if (selectAllInput != undefined) selectAllInput.checked = compt == storage.joueurs.length;
            if (!flag) {
...
function buildInterfaceHeaderAccueil() {
    ...
    var exp = MH.makeButton({
        type: "click",
        func: storage.export.bind(storage)
    }/*, "export"*/);
...
function validModificationContraintes() {
    ...
    for (var i = 0; i < contraintes.length; i++) {
        currentContrainte = {
            "name": storage.tournoi.contraintes[i].name,
            "title": storage.tournoi.contraintes[i].title,
            "desc": storage.tournoi.contraintes[i].desc,
            "actif": contraintes[i].querySelector("input").checked,
            "disabled": storage.tournoi.contraintes[i].disabled,
        }
        retourContraintes.push(currentContrainte);
        if (currentContrainte.name == "LIMITPOINT") {
            storage.updateTournoi({ "limitPoint": contraintes[i].querySelector("#limitPoint").value });
        }
    }
    storage.updateContraintes(retourContraintes);
    selectPage(pages.MODIFICATION_PREPARATION);
}
...
function deleteJoueur() {
    storage.deleteJoueur(currentEditionId);
...
function updateSelectJoueur(evt) {
    ...
    for (var i = 1; i < list.length; i++) {
        storage.updateJoueur(i - 1, { "selected": list[i].checked });
    }
}
...
function monterContrainte(i) {
    if (i > 0) {
        var cDisabled = storage.tournoi.contraintes[i - 1].disabled;
        var movedElt = storage.tournoi.contraintes[i];
        storage.tournoi.contraintes.splice(i, 1);
        storage.tournoi.contraintes.splice(i - 1 - (cDisabled ? 1 : 0), 0, movedElt);
    }
    selectPage(pages.MODIFICATION_CONTRAINTES);
}
function descendreContrainte(i) {
    if (i < storage.tournoi.contraintes.length - 1) {
        var cDisabled = storage.tournoi.contraintes[i + 1].disabled;
        var movedElt = storage.tournoi.contraintes[i];
        storage.tournoi.contraintes.splice(i, 1);
        storage.tournoi.contraintes.splice(i + 1 + (cDisabled ? 1 : 0), 0, movedElt);
...
function refreshMatch(domMatch, matchIndex) {
    ...
    const firstTeamThirdSet = domMatch.querySelector(".firstTeamThirdSet");
    const secondTeamFirstSet = domMatch.querySelector(".secondTeamFirstSet");
    ...
    storage.tournoi.contraintes.forEach(constraint => {
        if (constraint.actif && !constraint.disabled) {
            ...
            if (constraint.name == "ISOSEXE") {
                ...
                match.firstTeam.forEach(player => {
                    if (player.genre.value == genreListe.HOMME.value) {
                        nbHommeFirstTeam++;
                    }
                });
                match.secondTeam.forEach(player => {
...
function computeLeaderboards() {
    ...
    storage.tournoi.tours.flatMap(turn => turn.matchs).forEach(match => {
        ...
        if (matchWinner != null) {
            match.firstTeam.forEach(player => {
                if (player.ranked === undefined || player.ranked) {
...
function getSetWinner(setPoints, firstTeamScore, secondTeamScore) {
    ...
    if (setFinished) {
        if (firstTeamScore > secondTeamScore) {
            return SET_SCORE_FIRST_TEAM_KEY
        } else {
            return SET_SCORE_SECOND_TEAM_KEY
        }
    } else {
        return null
    }
